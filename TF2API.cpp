...
std::vector<std::pair<std::string, float>> getDPM() {
    const std::string steamID3 = convertSteamID64ToSteamID3("76561198930130823"); 
    ...
    for (size_t start = 0; start < totalLogs; start += batchSize) {
        ...
        for(int i = 0; i < curBatchSize; i++) {        
            ...
            try {
                ...
                int userDPM = jsonResponse["players"][steamID3]["dapm"];
                ...
                if (!allPlayerDPM.empty() && jsonResponse["players"][steamID3]["class_stats"][0]["type"] != "medic") { 
